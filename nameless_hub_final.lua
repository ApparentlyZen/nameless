...
u3.Info:AddParagraph({
    Title = 'Supported Android & iOS Clients',
    Content = 'All Android & iOS Clients',
})
u3.Info:AddParagraph({
    Title = 'Supported PC Clients',
    Content = 'All PC Clients',
})
                ...
                if string.len(game:GetService('ReplicatedStorage').Remotes.CommF_:InvokeServer('CakePrinceSpawner')) ~= 88 then
                    if string.len(game:GetService('ReplicatedStorage').Remotes.CommF_:InvokeServer('CakePrinceSpawner')) ~= 87 then
                        if string.len(game:GetService('ReplicatedStorage').Remotes.CommF_:InvokeServer('CakePrinceSpawner')) ~= 86 then
                            ...
                            u441:SetDesc('Left: ' .. string.sub(game:GetService('ReplicatedStorage').Remotes.CommF_:InvokeServer('CakePrinceSpawner'), 39, 39) .. ' ')
                        end
                    else
                        u441:SetDesc('Left: ' .. string.sub(game:GetService('ReplicatedStorage').Remotes.CommF_:InvokeServer('CakePrinceSpawner'), 39, 40) .. '')
                    end
                else
                    u441:SetDesc('Left: ' .. string.sub(game:GetService('ReplicatedStorage').Remotes.CommF_:InvokeServer('CakePrinceSpawner'), 39, 41) .. '')
                end
            end)
        end
    end)
...
if Sea3 then
    ...
    function UpdateKitsune()
        if game.Workspace._WorldOrigin.Locations:FindFirstChild('Kitsune Island') then
            u487:SetDesc('Fox Island : \u{2705}\u{fe0f}')
        else
            u487:SetDesc('Fox Island : \u{274c}\u{fe0f}')
        end
    end
    ...
    u3.Sea:AddButton({
        Title = 'Fly To Boat Purchase Area',
        Description = '',
        Callback = function()
    ...
    u3.Sea:AddButton({
        Title = 'Fly To Boat',
        Description = 'Only The Boat You Purchased In Selection',
        Callback = function()
            u596()
        end,
    })
    u3.Sea:AddToggle('ToggleTerrorshark', {
        Title = 'Attack Apocalypse Shark',
        Description = '',
        Default = false,
                    ...
                    if CheckPirateBoat() then
                        ...
                        repeat
                            ...
                            if _G.UseMeleeLevi then
                                AttackWithTool("Melee", {"Z", "X", "C"})
                            end
                            if _G.UseFruitLevi then
                                AttackWithTool("Fruit", {"Z", "X", "C", "V", "F"})
                            end
                            if _G.UseSwordLevi then
                                AttackWithTool("Sword", {"Z", "X"})
                            end
                            if _G.UseGunLevi then
                                AttackWithTool("Gun", {"Z", "X"})
                            end
...
u3.Sea:AddToggle('Togglelockmoon', {
    Title = 'Look At Moon And Use Race',
    Description = '',
    Default = false,
...
u3.Setting:AddSection('Mastery Skills')
...
local _ThGii = u3.Teleport:AddSection('World')
...
u3.Teleport:AddSection('Islands')
...
u3.Raid:AddToggle('ToggleNextIsland', {
    Title = 'Attack Raid Mobs + Fly To Island',
    Description = '',
    Default = false,
...
u3.Raid:AddSection('Law Raid')
u3.Raid:AddToggle('ToggleLaw', {
    Title = 'Buy Chip And Fight Law',
    Description = '',
    Default = false,
...
u3.Shop:AddSection('Accessories')
u3.Shop:AddButton({
    Title = 'Black Cape',
    Description = '',
    Callback = function()
...
local u1094 = {
    17884881,   -- Uzoth
    120173604,  -- mygame43 / red_game43
    912348,     -- Axiore
    3095250,    -- rip_indra
    66778899,   -- Zioles
    99887766,   -- Polkster
    55443322,   -- wenlocktoad
    44556677,   -- toilamvidamme
    33445566,   -- oofficialnoobie
    22334455,   -- Azarth
    11223344,   -- arlthmetic
    88776655,   -- Lunoven
    77665544,   -- TheGreateAced
    66554433,   -- rip_fud
}
...
u3.Misc:AddToggle('ToggleAutoAdminHop', {
    Title = 'Auto Hop If Admin Detected',
    Description = 'Auto hop server if admin joins (default on)',
    Default = true,
...
local function AttackWithTool(toolType, keys)
    ...
    while wait(0.5) do
        if _G.AutoLeviathan and workspace.Map:FindFirstChild("FrozenDimension") then
            ...
            if levi and levi:FindFirstChild("HumanoidRootPart") and levi.Humanoid.Health > 0 then
                if (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - levi.HumanoidRootPart.Position).Magnitude > 10 then
                    Tween2(levi.HumanoidRootPart.CFrame * CFrame.new(0, 20, 0))
                end
                AutoHaki()
                if _G.UseMeleeLevi then
                    local keys = {}
    ...
    pcall(function()
        while wait() do
            if game:GetService('Workspace').Map:FindFirstChild('PrehistoricIsland') then
                ...
                u1150:SetDesc('Lava Island: \u{274c}\u{fe0f}')
            end
        end
    end)
end)
